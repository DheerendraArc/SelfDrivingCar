...
class RCControl(object):
    ...
    def __init__(self):
        self.serial_port = serial.Serial('/dev/cu.usbmodem1411', 115200, timeout=1)
...
class ObjectDetection(object):
    ...
    def detect(self, cascade_classifier, gray_image, image):
        ...
        cascade_obj = cascade_classifier.detectMultiScale(
            ...
            minNeighbors=5,
            minSize=(30, 30),
            flags=cv2.CASCADE_SCALE_IMAGE  #cv is not working proper for python 2.7.1
        )
...
class VideoStreamHandler(socketserver.StreamRequestHandler):
    ...
    #nn = NeuralNetwork('mlp_xml/mlp.xml')
    #nn.create()
    #prediction = nn.predict('training_images')
    model = NeuralNetwork()
    model.create()
    ...
    # cascade classifiersNeuralNetwork
        ...
        try:
            while True:
                ...
                if first != -1 and last != -1:
                    ...
                    gray = cv2.imdecode(np.fromstring(jpg, dtype=np.uint8), 0) #cv2.CV_LOAD_IMAGE_GRAYSCALE
...
class ThreadServer(object):
    ...
    distance_thread = threading.Thread(target=server_thread2, args=('192.168.43.20', 8002))
    distance_thread.start()
    video_thread = threading.Thread(target=server_thread('192.168.43.20', 8000))
    video_thread.start()
